                             ...
                             Number *dst)
{
  ...
  if(row<N){
    ...
    const size_t matrix_width = 27;
    size_t idx_block = row / warp;
    size_t idx_offset = idx_block * warp * matrix_width;
    size_t idx_row = row % warp;


    for(size_t i=0; i<matrix_width; i++){
      sum += values[idx_offset + idx_row + i*warp] * src[column_indices[idx_offset + idx_row + i*warp]];
    }
    dst[row] = sum;
  }
...
#pragma omp parallel for
  ...
  void transmethod()
  {
    const size_t warp = 32;
    const size_t matrix_width = 27;
    const size_t new_array_length = n_rows * matrix_width;

    Number *new_values = new Number[new_array_length];
    unsigned int *new_col_indices = new unsigned int[new_array_length];
    ...
    row_starts[n_rows] = new_array_length;
    n_global_nonzero_entries = new_array_length;
    
    row_starts[0] = 0;
    for(size_t i=1; i<n_rows; i++){
      row_starts[i] = row_starts[i-1] + matrix_width;
    }
