                             ...
                             Number *dst)
{
  ...
  //original version for part 1, wrong
  ...
  //   size_t idx_offset = 0;
  //   for(size_t i=0; i<idx_blocks;i++){idx_offset = idx_offset + warp*row_starts[i*warp];}
  //   size_t idx_row = row % warp;

  //   for (size_t idx = idx_offset + idx_row; idx < idx_offset + idx_row + (row_starts[row]-1)*warp; idx=idx+warp)
  //   {
  //     sum += values[idx] * src[column_indices[idx]];
  //   }
  //   dst[row] = sum;
...
//Abort, do it in cpu
template <typename Number>
__global__ void trans(std::size_t n_rows,
                      std::size_t *row_starts,
                      unsigned int *column_indices,
                      Number *values)
{

  // for(size_t i=0; i<n_rows; i++){row_starts[i] = block_width[i/warp];}
...
public:
      ...
      memory_space(memory_space)
  {
    ...
    std::size_t *row_starts = new std::size_t[n_rows + 1];
...
#pragma omp parallel for
    ...
    for (unsigned int row = 0; row < n_rows; ++row)
      {row_starts[row + 1] = row_starts[row] + row_lengths[row];
       printf("row_starts[%u + 1] = %lu\n", row, row_starts[row + 1]);
       printf("row_lengths[row] = %u\n", row_lengths[row]);
      }
    ...
    if (memory_space == MemorySpace::CUDA)
      {
        ...
        printf("enter cuda copy\n");
        printf("n_blocks = %u, block_size = %u, n_entries = %lu\n", n_blocks, block_size, n_entries);
...
#pragma omp parallel for
  ...
  void transmethod()
  {
    ...
    for(int i=0; i<n_blocks; i++){printf("block_width[i] = %lu\n", block_width[i]);}
    ...
    for (size_t row = 0; row < n_rows; ++row)
      {
        size_t idx_blocks = row / warp;
        size_t idx_offset = 0;
        for(size_t i=0; i<idx_blocks;i++){idx_offset = idx_offset + 32*block_width[i];}
        size_t idx_row = row % 32;
        size_t idx_idx = 0;
        
        for (size_t idx = row_starts[row]; idx < row_starts[row + 1]; ++idx)
        {
          new_values[idx_offset + idx_row + idx_idx*32] = values[idx];
          new_col_indices[idx_offset + idx_row + idx_idx*32] = column_indices[idx];
          idx_idx = idx_idx + 1;
        }
      }
    ...
    row_starts[0] = 0;
    for(size_t i=1; i<n_rows; i++){row_starts[i] = row_starts[i-1] + block_width[(i-1)/warp];printf("trans row_starts = %lu\n", row_starts[i]);}
