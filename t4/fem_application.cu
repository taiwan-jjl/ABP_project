...
#pragma omp for collapse(2)
  ...
  //sparse.trans();
  //change my mind, dont do it here
...
void run_test(const long long N, const long long n_repeat)
{
  ...
  //need to go into matrix_dev to access public and private variables
  // matrix.transmethod();    printf("copy to dev here\n");
...
#pragma omp critical
...

  {
    ...
    const auto t1 = std::chrono::steady_clock::now();printf("here-test\n");
    for (unsigned long long rep = 0; rep < n_repeat; ++rep)
      {matrix_dev.apply(src_device, dst_device);printf("here-test\n");}
    cudaDeviceSynchronize();
